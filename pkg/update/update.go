...
var (
	lastUpdateCheckFilePath = constants.MakeFunktionPath("last_update_check")
)
...
func MaybeUpdateFromGithub(output io.Writer) {
	...
	MaybeUpdate(output, githubOwner, githubRepo, githubRepo, lastUpdateCheckFilePath, localVersion, true)
}

func MaybeUpdate(output io.Writer, githubOwner, githubRepo, binaryName, lastUpdatePath string, localVersion semver.Version, viewLatest bool) {
...
func shouldCheckURLVersion(filePath string) bool {
	if !viper.GetBool(config.WantUpdateNotification) {
		fmt.Println("No update notification required")
		return false
	}
